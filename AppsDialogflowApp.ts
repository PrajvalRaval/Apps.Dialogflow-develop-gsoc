...
export class AppsDialogflowApp extends App implements IPostMessageSent {
    ...
    public async executePostMessageSent(
        ...
        const SettingBotUsername: string = await this.getAppSetting(
            read,
            'Lc-Bot-Username',
        );
        ...
        const clientEmail = await this.getAppSetting(
            read,
            'Dialogflow-Client-Email',
        );
        const privateKey = await this.getAppSetting(
            read,
            'Dialogflow-Private-Key',
        );
        const projectId = await this.getAppSetting(
            read,
            'Dialogflow-Project-Id',
        );
        const sessionId = 'test2';

        const dialogflowWrapper: DialogflowWrapper = new DialogflowWrapper(
            clientEmail,
            privateKey,
        );
        ...
        const httpRequestContent: IHttpRequest = buildDialogflowHTTPRequest(
            message.text,
        );
        ...
        try {
            ...
            if (responseJSON.queryResult) {
                if (responseJSON.queryResult.fulfillmentMessages[1]) {
                    const BotResponse =
                        responseJSON.queryResult.fulfillmentText;
                    const responseActions =
                        responseJSON.queryResult.fulfillmentMessages[1].payload
                            .actions;
                    ...
                    responseActions.map((item) => {
                        if (item.type === 'button') {
                            ...
                            buttonSendMessageModel.text = item.text;
                            buttonSendMessageModel.msg = item.msg;

                            actions.push(buttonSendMessageModel);
                        }
                    });
                    ...
                    const builderattachment = modify
                        .getNotifier()
                        .getMessageBuilder();
                    builderattachment
                        ...
                        .setAttachments([test])
                        .setSender(LcAgent);
                    await modify.getCreator().finish(builderattachment);
                } else {
                    const BotResponse =
                        responseJSON.queryResult.fulfillmentText;
    ...
    public async getAppSetting(read: IRead, id: string): Promise<any> {
        return (await read.getEnvironmentReader().getSettings().getById(id))
            .value;
    }
    ...
    public async onSettingUpdated(
        ...
        const clientEmail: string = await this.getAppSetting(
            read,
            'Dialogflow-Client-Email',
        );
        const privateKey: string = await this.getAppSetting(
            read,
            'Dialogflow-Private-Key',
        );
        ...
        if (clientEmail.length === 0 || privateKey.length === 0) {
            this.getLogger().error(
                'Client Email or Private Key Field cannot be empty',
            );
            return;
        }
